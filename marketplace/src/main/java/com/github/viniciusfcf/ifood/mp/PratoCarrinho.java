...
public class PratoCarrinho {
	...
	public static Uni<Long> save(PgPool client, String cliente, Long prato){
		return client.preparedQuery("INSERT INTO prato_cliente (clientem prato) VALUES ($1, $2) RETURNING (cliente)").execute(
	...
	public static Uni<List<PratoCarrinho>> findCarrinho(PgPool client, String cliente){
		return client.preparedQuery("SELECT * FROM prato_cliente WHERE cliente = $1 ").execute(
				Tuple.of(cliente))
				.map(pgRowSet ->{
					List<PratoCarrinho> list = new ArrayList<PratoCarrinho>(pgRowSet.size());
	...
	public static PratoCarrinho toPratoCarrinho(Row row) {
	...
	public static Uni<Boolean> delete(PgPool client, String cliente){
		return client.preparedQuery("DELETE FROM prato_cliente WHERE cliente = $1").execute(
				Tuple.of(cliente))
